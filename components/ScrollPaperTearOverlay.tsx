...
export const ScrollPaperTearOverlay: React.FC = () => {
  ...
  // 60 FPS Canvas Renderer: Entire area UNDER white jagged line is 100% KAMNA-PORTFOLIO background (#0A0A0A + Top-Right Neon Glow)
  const renderCanvas = useCallback(
    (timestamp: number) => {
      ...
      // 1. ENTIRE AREA UNDER TEAR LINE: KAMNA-PORTFOLIO MATTE BLACK (#0A0A0A)
      ...
      // 2. TOP PAPER LAYER (#0D0D2E) CLIPPED STRICTLY ABOVE THE JAGGED LINE
      const pointsCount = noisePointsRef.current.length ? noisePointsRef.current.length - 1 : 30;
      const topY = gh * 0.15;
      const botY = gh * 0.45;
      // Cut line sweeps from 0 to gw * 0.88 so the jagged paper line remains permanently resting in the upper section
      const cutX = t * (gw * 0.88);
      ...
      // Clip Path for upper-right paper triangle ABOVE the tear line
      ctx.save();
      ...
      // 3. CRISP WHITE JAGGED PAPER TEAR LINE & 3D DROP SHADOWS
  ...
  return (
    ...
    >
      ...
      {/* PHASE 4: FULL-SCREEN CANVAS PAPER TEAR TRANSITION (REVEALS 100% KAMNA-PORTFOLIO MATTE BLACK #0A0A0A + PINK NEON AURA BELOW WHITE JAGGED LINE) */}
      {isStatementScreen && (
        <section
          ref={tearSectionRef}
